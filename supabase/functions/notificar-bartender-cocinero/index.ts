// supabase/functions/notificar-bartender-cocinero/index.ts
...
function makeHtml(pedidoId: number, mesaNumero: number, total: number, tiempoEstimado: number) {
  const titulo = "Pedido confirmado – El Tridente de la Gloria";

  const lead = `El pedido de la Mesa ${mesaNumero} ha sido confirmado por el mozo. Total: $${total} - Tiempo estimado: ${tiempoEstimado} minutos.`;

  const nombreCompleto = `Bartender/Cocinero`;

  // Preheader para inbox (oculto)
  const preheader = `Pedido confirmado Mesa ${mesaNumero} - $${total} - ${tiempoEstimado}min`;
        ...
        <table role="presentation" width="600" cellspacing="0" cellpadding="0"
          ...
          <tr>
            <td style="padding:22px 24px; background:${BRAND_BG};">
              ...
              <p style="margin:16px 0 0; color:#333; font-size:14px;">
                <strong>¡Es hora de preparar el pedido!</strong> Revisá los detalles en la app.
              </p>
            </td>
          </tr>
...
Deno.serve(async (req: Request) => {
  ...
  const { pedidoId, mesaNumero, total, tiempoEstimado } = body;
  ...
  // Obtener emails de bartenders y cocineros desde Supabase
  ...
  // Obtener emails de bartenders y cocineros
  const staffResponse = await fetch(`${supabaseUrl}/rest/v1/usuarios?tipo=in.(bartender,cocinero)&select=email`, {
    headers: {
  ...
  if (!staffResponse.ok) {
    return new Response(
      JSON.stringify({ ok: false, error: "Failed to fetch bartenders/cocineros" }),
      { status: 500, headers: { "Content-Type": "application/json", ...cors } }
    );
  }
  ...
  if (!staff || staff.length === 0) {
    return new Response(
      JSON.stringify({ ok: false, error: "No bartenders/cocineros found" }),
      { status: 404, headers: { "Content-Type": "application/json", ...cors } }
    );
  }

  const subject = "Pedido confirmado – El Tridente de la Gloria";

  let html = makeHtml(pedidoId, mesaNumero, total, tiempoEstimado);
  const text = `Hola Bartender/Cocinero,
El pedido de la Mesa ${mesaNumero} ha sido confirmado por el mozo.
...
— El Tridente de la Gloria`;
  ...
  // Payload SendGrid - Enviar a todos los bartenders y cocineros
  const payload: Record<string, unknown> = {
    personalizations: staff.map(member => ({
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ ok: true, staffNotificados: staff.length }),
